...
enum DotLottieColor {
    ...
    static func nsColor(_ colorString: String) -> NSColor? {
    ...
    static func ciColor(_ colorString: String) -> CIColor? {
        guard let argb = argb(colorString) else { return nil }
        return CIColor(
            red: CGFloat((argb & 0x00FF_0000) >> 16) / 255.0,
            green: CGFloat((argb & 0x0000_FF00) >> 8) / 255.0,
...
final class DotLottieSoftwareRenderer: DotLottieFlutterRenderer {
    ...
    func mount(in container: NSView) {
        let hosting = NSHostingController(rootView: animation.view() as DotLottieView)
    ...
    func setBackgroundColor(hex: String) {
        guard let color = DotLottieColor.ciColor(hex) else { return }
        animation.setBackgroundColor(bgColor: CIImage(color: color))
    }
